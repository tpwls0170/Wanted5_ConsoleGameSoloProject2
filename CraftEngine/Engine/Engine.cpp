...
namespace Craft
{
	...
	Engine::Engine()
	{
		...
		renderer = std::make_unique<Renderer>(
			Vector2(setting.consolwidth, setting.consolheight)
		);
	...
	void Engine::LoadEngineSetting()
	{
		...
		while (token)
		{
			...
			else if (strcmp(key, "consolwidth") == 0)
			{
				sscanf_s(token, "consolwidth = %d", &setting.consolwidth);
			}
			else if (strcmp(key, "consolheight") == 0)
			{
				sscanf_s(token, "consolheight = %d", &setting.consolheight);
			}
	...
	void Engine::SetConsoleSize()
	{
		std::string command =
			"mode con: cols=" +
			std::to_string(setting.consolwidth) +
			" lines=" +
			std::to_string(setting.consolheight);
